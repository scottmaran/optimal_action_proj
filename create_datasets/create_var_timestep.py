...
for index in range(0, num_plays):
    ...
    # skip NUM_FRAMES_TO_SKIP frames
    try:
        starting_events = ['kick_received', 'punt_received']
        starting_frame = play_df.query("event in @starting_events").frameId.values[0]
        ending_events = ['tackle', 'out_of_bounds', 'touchdown', 'fumbled']
        ending_frame = play_df.query("event in @ending_events").frameId.values[0]
        
        if (ending_frame - starting_frame > NUM_FRAMES_TO_SKIP):
            frames = np.arange(starting_frame, ending_frame, NUM_FRAMES_TO_SKIP)
            play_df = play_df.query("frameId in @frames", inplace=False).copy()
        else:
            print(f"play too short. Only {ending_frame - starting_frame} frames long")
            continue
    except:
        print("error with play- could not find start of end frame")
    ...
    for time_recorded_index, time_string in enumerate(all_time_steps):
        ...
        try:
            ...
            rowDict["timeRecordedIndex"] = time_recorded_index
            rowDict["time"] = time_string
            rowDict["football_pos"] = football_pos.astype(float).round(3)
            # add current state
            ...
            # add reward
            try:
                rowDict['reward'] = time_play_df.loc[0,'reward'].round(3)
            except:
...
full_train_df.to_pickle(f"datasets/bc_dataset_{year_str}_{NUM_FRAMES_TO_SKIP}_secs.pkl")
